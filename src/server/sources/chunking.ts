...
export function rankClaimChunks(input: { sourceId: string; text: string; claim: string; location?: string; maxChunks?: number; maxChars?: number }): RankedChunk[] {
  ...
  const ranked = paragraphs.map((text, index) => { const paragraphWords = words(text); const overlap = [...claimWords].filter((word) => paragraphWords.has(word)).length; return { text, index, score: claimWords.size ? overlap / claimWords.size : 0 }; }).filter(({ score }) => score > 0).sort((a, b) => b.score - a.score || a.index - b.index);
