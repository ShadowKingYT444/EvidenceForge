...
function rightsDecision(rights: SourceRightsRequest | undefined, now: string) {
  return { mayStore: rights?.mayStore ?? "allowed", mayDisplay: rights?.mayDisplay ?? "allowed", maySendToModel: rights?.maySendToModel ?? "allowed", basis: rights?.permissionBasis ?? "OpenAlex-reported open-access location; researcher approved import", checkedAt: rights?.checkedAt ?? now } as const;
}
...
export async function importOpenAlexWork(input: { openAlexId: string; claims: readonly string[]; rights?: SourceRightsRequest }, dependencies: OpenAlexImportServiceDependencies): Promise<ImportedSource> {
  ...
  if (!dependencies.apiKey.trim()) throw new Error("OpenAlex API key is required");
  const now = (dependencies.now ?? (() => new Date()))().toISOString();
  const fetcher = dependencies.fetch ?? fetch;
  const response = await fetcher(`https://api.openalex.org/works/${encodeURIComponent(id)}?api_key=${encodeURIComponent(dependencies.apiKey)}`, { headers: { accept: "application/json" } });
  ...
  if (rights.mayStore !== "allowed" || rights.maySendToModel !== "allowed") {
...
export function createPastedSource(input: { id: string; title: string; text: string; claims: readonly string[]; originalInput: string; authors?: readonly string[]; year?: number | null; venue?: string | null; rights?: SourceRightsRequest; now?: Date }): ImportedSource {
  ...
  const unique = [...new Map(chunks.map((chunk) => [chunk.contentHash, chunk])).values()].slice(0, 32);
  const warnings = rights.mayStore === "allowed" ? [] : ["Pasted text is not stored because storage permission is not allowed."];
  const storedText = rights.mayStore === "allowed" ? unique.map(({ text: value }) => value).join("\n\n") : "";
  const source = SourceRecordSchema.parse({ id: input.id, originalInput: input.originalInput, canonicalDoi: null, canonicalUrl: null, doiResolution: { syntax: "not_provided", resolution: "not_checked", registrationAgency: null, checkedAt: null }, bibliographicMetadata: { title: input.title.trim(), authors: [...(input.authors ?? [])], year: input.year ?? null, venue: input.venue ?? null, studyType: null }, access: { origin: "user_import", contentScope: unique.length && rights.mayStore === "allowed" ? "user_excerpt" : "metadata_only", provider: "researcher", version: null, location: "pasted excerpt", retrievedAt: now }, rights, contentHash: sha256(storedText), metadataVerification: { status: "not_checked", method: "Researcher-provided metadata", checkedAt: null, fieldDiffs: [] }, integrityNotices: [], mergedSourceIds: [], warnings });
  return { source, chunks: unique.map((chunk) => SourceChunkSchema.parse({ id: chunk.id, sourceId: input.id, text: chunk.text, location: chunk.location, contentHash: chunk.contentHash, displayPermission: rights.mayDisplay })), warnings };
}
