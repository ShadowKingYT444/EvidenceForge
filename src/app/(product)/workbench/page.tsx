...
import { buildPacketReviewModel, isPacketReviewScenario } from "@/features/workbench/packet-review-state";
...
export default async function WorkbenchPage({ searchParams }: WorkbenchPageProps) {
  ...
  const packetReview = buildPacketReviewModel(run, !query.runId && isPacketReviewScenario(query.packet) ? query.packet : "frozen");
  ...
  const finalDecision = query.runId && revision
    ? run.finalDecision ? { kind: "recorded" as const, run } : { kind: "process_local" as const, run, revision }
  ...
  return <WorkbenchShell model={model} packetReview={packetReview} evidenceMatrix={evidenceMatrix} conclusionsGap={conclusionsGap} experimentProtocol={experimentProtocol} objectionDisposition={objectionDisposition} finalDecision={finalDecision} initialEvidenceId={query.evidence ?? null} allowSimulatedStates={!query.runId} initialStage={initialStage} />;
}
