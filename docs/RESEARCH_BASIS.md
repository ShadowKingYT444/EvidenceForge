...
|---|---|---|
| Primary | Groq `openai/gpt-oss-120b` | `confirmed` transport and strict one-field JSON-schema smoke pass |
| Reviewer | NVIDIA NIM `nvidia/nemotron-3-super-120b-a12b` | `confirmed` transport and prompted JSON-object smoke pass with thinking disabled |

Groq documents strict JSON Schema support for GPT-OSS 20B/120B, including required fields and closed objects. See [Groq Structured Outputs](https://console.groq.com/docs/structured-outputs). The application still validates every response with Zod and records refusals and failures.

NVIDIA documents Nemotron 3 Super as a March 2026 120B/12B-active Mamba-2 + MoE + attention hybrid with configurable thinking and science/RAG training. See the [model reference](https://docs.api.nvidia.com/nim/reference/nvidia-nemotron-3-super-120b-a12b) and [hosted endpoint](https://build.nvidia.com/nvidia/nemotron-3-super-120b-a12b). The hosted endpoint accepted JSON object mode in a live smoke test, but strict schema guarantees are `unverified`; use prompt-guided JSON, local Zod/invariant validation, and at most one logged repair attempt.
...
Current official pages conflict: the [Devpost overview](https://reverie-hacks-2026.devpost.com/) and [event site](https://www.reveriehacks.org/) say August 2–17, while the [rules](https://reverie-hacks-2026.devpost.com/rules) say August 3–17. The project conservatively uses August 3 as its implementation boundary, and repository provenance records no product implementation before August 6. The ML track says four files but lists only workflow PNG, samples, and documentation; the unnamed fourth file remains a final release blocker and is never inferred.
